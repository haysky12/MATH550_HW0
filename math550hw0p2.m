...
ddf = centralDiff2(y,h); % numerical approx.

% actual 2nd derivative for comparison
ddy = @(x) -exp(sin(x)) .* (sin(x) - cos(x).^2);
ddyv = ddy(x);

% graph
hold on
p2 = plot(x, ddyv, 'Color', '#FEB82C'); % gold
p1 = plot(x, ddf, 'o', 'Color', "#AC2694"); % dark magenta
